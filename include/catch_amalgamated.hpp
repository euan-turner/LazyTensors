...
// mask themselves as GCC should be ignored.
    ...
    !defined(__LCC__) && !defined(__NVCOMPILER)
...
#define CATCH_CONFIG_NEW_CAPTURE
    ...
    !defined(CATCH_CONFIG_DISABLE_EXCEPTIONS) && !defined(CATCH_CONFIG_NO_DISABLE_EXCEPTIONS)
...
struct ResultWas {
    enum OfType {
        ...
        Info = 1,
        Warning = 2,
        // TODO: Should explicit skip be considered "not OK" (cf. isOk)? I.e., should it have the
        // failure bit?
        ExplicitSkip = 4,
...
class IResultCapture {
   public:
    ...
    virtual bool sectionStarted(StringRef sectionName, SourceLineInfo const& sectionLineInfo,
                                Counts& assertions) = 0;
    virtual void sectionEnded(SectionEndInfo&& endInfo) = 0;
    virtual void sectionEndedEarly(SectionEndInfo&& endInfo) = 0;

    virtual IGeneratorTracker* acquireGeneratorTracker(StringRef generatorName,
    ...
    virtual void handleMessage(AssertionInfo const& info, ResultWas::OfType resultType,
                               std::string&& message, AssertionReaction& reaction) = 0;
...
CompleteType_t<FunctionReturnType<Fun, Args...>> complete_invoke(Fun&& fun, Args&&... args) {
    return CompleteInvoker<FunctionReturnType<Fun, Args...>>::invoke(CATCH_FORWARD(fun),
                                                                     CATCH_FORWARD(args)...);
}
...
struct ExecutionPlan {
    ...
    std::vector<FDuration> run(const IConfig& cfg, Environment env) const {
        // warmup a bit
        Detail::run_for_at_least<Clock>(std::chrono::duration_cast<IDuration>(warmup_time),
                                        warmup_iterations,
...
EnvironmentEstimate estimate_clock_resolution(int iterations) {
    auto r =
        run_for_at_least<Clock>(clock_resolution_estimation_time, iterations, &resolution<Clock>)
            .result;
    return {
...
struct StringMaker {
    ...
    static std::enable_if_t<::Catch::Detail::IsStreamInsertable<Fake>::value, std::string> convert(
        const Fake& value) {
...
template <typename R>
struct StringMaker<
    R, std::enable_if_t<is_range<R>::value && !::Catch::Detail::IsStreamInsertable<R>::value>> {
...
class ReporterSpec {
   ...
   public:
    ReporterSpec(std::string name, Optional<std::string> outputFileName,
                 Optional<ColourMode> colourMode, std::map<std::string, std::string> customOptions);
...
class ParserRefImpl : public ComposableParserImpl<DerivedT> {
   ...
   public:
    ...
    template <typename T,
              typename = typename std::enable_if_t<!Detail::is_unary_function<T>::value>>
    ParserRefImpl(T& ref, StringRef hint)
        : m_ref(std::make_shared<BoundValueRef<T>>(ref)), m_hint(hint) {}

    template <typename LambdaT,
              typename = typename std::enable_if_t<Detail::is_unary_function<LambdaT>::value>>
...
#define CATCH_DEFINE_COMPARABLE_TRAIT(id, op)                                                     \
    ...
    template <typename T, typename U>                                                             \
    struct is_##id##_comparable<T, U, void_t<decltype(std::declval<T>() op std::declval<U>())>>   \
        : std::true_type {};                                                                      \
    template <typename, typename = void>                                                          \
    struct is_##id##_0_comparable : std::false_type {};                                           \
    template <typename T>                                                                         \
    struct is_##id##_0_comparable<T, void_t<decltype(std::declval<T>() op 0)>> : std::true_type { \
    };
...
#define CATCH_INTERNAL_DEFINE_EXPRESSION_EQUALITY_OPERATOR(id, op)                               \
    ...
    constexpr friend auto operator op(ExprLhs&& lhs, RhsT rhs)                                   \
        ->std::enable_if_t<                                                                      \
            Detail::conjunction<                                                                 \
                ...
                Detail::is_eq_0_comparable<LhsT>, /* We allow long because we want `ptr op NULL` \
                                                     to be accepted */                           \
    ...
    constexpr friend auto operator op(ExprLhs&& lhs, RhsT rhs)                                   \
        ->std::enable_if_t<                                                                      \
            Detail::conjunction<                                                                 \
                Detail::negation<Detail::is_##id##_comparable<LhsT, RhsT>>,                      \
                Detail::is_eq_0_comparable<RhsT>, /* We allow long because we want `ptr op NULL` \
                                                     to be accepted */                           \
...
#define CATCH_INTERNAL_DEFINE_EXPRESSION_OPERATOR(op)                               \
    template <typename RhsT>                                                        \
    constexpr friend auto operator op(ExprLhs&& lhs, RhsT&& rhs)                    \
        ->std::enable_if_t<!capture_by_value<Detail::RemoveCVRef_t<RhsT>>::value,   \
                           BinaryExpr<LhsT, RhsT const&>> {                         \
        return {static_cast<bool>(lhs.m_lhs op rhs), lhs.m_lhs, #op##_sr, rhs};     \
    }                                                                               \
    template <typename RhsT>                                                        \
...
struct Decomposer {
    template <typename T,
              std::enable_if_t<!capture_by_value<Detail::RemoveCVRef_t<T>>::value, int> = 0>
...
#define INTERNAL_CATCH_TEST_CASE_METHOD2(TestName, ClassName, ...)                               \
    ...
    const Catch::AutoReg INTERNAL_CATCH_UNIQUE_NAME(autoRegistrar)(                              \
        Catch::makeTestInvoker(&TestName::test), CATCH_INTERNAL_LINEINFO, #ClassName##_catch_sr, \
        Catch::NameAndTags{__VA_ARGS__}); /* NOLINT */                                           \
    }                                                                                            \
    CATCH_INTERNAL_STOP_WARNINGS_SUPPRESSION                                                     \
    void TestName::test()
#define INTERNAL_CATCH_TEST_CASE_METHOD(ClassName, ...)                                            \
    INTERNAL_CATCH_TEST_CASE_METHOD2(INTERNAL_CATCH_UNIQUE_NAME(CATCH2_INTERNAL_TEST_), ClassName, \
                                     __VA_ARGS__)
...
#define INTERNAL_CATCH_TEMPLATE_TEST_CASE_NO_REGISTRATION(Name, Tags, ...)                         \
    INTERNAL_CATCH_TEMPLATE_TEST_CASE_NO_REGISTRATION_2(                                           \
        ...
        INTERNAL_CATCH_UNIQUE_NAME(CATCH2_INTERNAL_TEMPLATE_TEST_), Name, Tags, typename TestType, \
        __VA_ARGS__)
#else
#define INTERNAL_CATCH_TEMPLATE_TEST_CASE_NO_REGISTRATION(Name, Tags, ...)                         \
...
#define INTERNAL_CATCH_TEMPLATE_TEST_CASE_SIG_NO_REGISTRATION(Name, Tags, Signature, ...)  \
    INTERNAL_CATCH_TEMPLATE_TEST_CASE_NO_REGISTRATION_2(                                   \
        INTERNAL_CATCH_UNIQUE_NAME(CATCH2_INTERNAL_TEMPLATE_TEST_),                        \
        INTERNAL_CATCH_UNIQUE_NAME(CATCH2_INTERNAL_TEMPLATE_TEST_), Name, Tags, Signature, \
        __VA_ARGS__)
#else
#define INTERNAL_CATCH_TEMPLATE_TEST_CASE_SIG_NO_REGISTRATION(Name, Tags, Signature, ...)  \
    INTERNAL_CATCH_EXPAND_VARGS(INTERNAL_CATCH_TEMPLATE_TEST_CASE_NO_REGISTRATION_2(       \
...
#define INTERNAL_CATCH_TEMPLATE_TEST_CASE_METHOD_NO_REGISTRATION(ClassName, Name, Tags, ...) \
    INTERNAL_CATCH_TEMPLATE_TEST_CASE_METHOD_NO_REGISTRATION_2(                              \
        ...
        INTERNAL_CATCH_UNIQUE_NAME(CATCH2_INTERNAL_TEMPLATE_TEST_), ClassName, Name, Tags,   \
...
#define INTERNAL_CATCH_TEMPLATE_PRODUCT_TEST_CASE(Name, Tags, ...)                          \
    INTERNAL_CATCH_TEMPLATE_PRODUCT_TEST_CASE2(                                             \
        INTERNAL_CATCH_UNIQUE_NAME(CATCH2_INTERNAL_TEMPLATE_TEST_),                         \
        INTERNAL_CATCH_UNIQUE_NAME(CATCH2_INTERNAL_TEMPLATE_TEST_), Name, Tags, typename T, \
        __VA_ARGS__)
#else
#define INTERNAL_CATCH_TEMPLATE_PRODUCT_TEST_CASE(Name, Tags, ...)                          \
            ...
            void reg_tests() {                                                                    \
                ...
                (void)expander{                                                                   \
                    (Catch::AutoReg(                                                              \
                         ...
                         Catch::StringRef(),                                                      \
                         Catch::NameAndTags{Name " - " INTERNAL_CATCH_STRINGIZE(TmplList) " - " + \
                                                std::to_string(index),                            \
                                            Tags}),                                               \
                     index++)...}; /* NOLINT */                                                   \
            }                                                                                     \
...
GeneratorWrapper<DecayedT> value(T&& value) {
    return GeneratorWrapper<DecayedT>(
        Catch::Detail::make_unique<SingleValueGenerator<DecayedT>>(CATCH_FORWARD(value)));
}
template <typename T>
...
#define GENERATE(...)                                                              \
    Catch::Generators::generate(                                                   \
        CATCH_INTERNAL_GENERATOR_STRINGIZE(INTERNAL_CATCH_UNIQUE_NAME(generator)), \
...
GeneratorWrapper<T> take(size_t target, GeneratorWrapper<T>&& generator) {
    return GeneratorWrapper<T>(
        Catch::Detail::make_unique<TakeGenerator<T>>(target, CATCH_MOVE(generator)));
}
...
GeneratorWrapper<T> repeat(size_t repeats, GeneratorWrapper<T>&& generator) {
    return GeneratorWrapper<T>(
        Catch::Detail::make_unique<RepeatGenerator<T>>(repeats, CATCH_MOVE(generator)));
}
...
GeneratorWrapper<T> map(Func&& function, GeneratorWrapper<U>&& generator) {
    return GeneratorWrapper<T>(Catch::Detail::make_unique<MapGenerator<T, U, Func>>(
...
#if defined(CATCH_CONFIG_INTERNAL_UINT128) && !defined(CATCH_CONFIG_NO_UINT128) && \
 ...
 */
constexpr ExtendedMultResult<std::uint64_t> extendedMultPortable(std::uint64_t lhs,
                                                                 std::uint64_t rhs) {
...
std::enable_if_t<sizeof(typename Generator::result_type) >= sizeof(TargetType), TargetType>
fillBitsFrom(Generator& gen) {
    ...
    static_assert(Generator::min() == 0 && Generator::max() == static_cast<gresult_type>(-1),
                  "Generator must be able to output all numbers in its result type (effectively it "
                  "must be a random bit generator)");
...
class uniform_integer_distribution {
    ...
    // How many different values are there in [a, b]. a == b => 1, can be 0 for distribution over
...
template <typename InputIterator, typename InputSentinel,
          typename ResultType =
              std::remove_const_t<typename std::iterator_traits<InputIterator>::value_type>>
...
struct ReporterConfig {
    ReporterConfig(IConfig const* _fullConfig, Detail::unique_ptr<IStream> _stream,
...
struct TestCaseStats {
    TestCaseStats(TestCaseInfo const& _testInfo, Totals const& _totals, std::string&& _stdOut,
...
class ITestCaseRegistry {
   public:
    ...
    // TODO: this exists only for adding filenames to test cases -- let's expose this in a saner way
    // later
...
constexpr ForwardIter find_sentinel(ForwardIter start, Sentinel sentinel, T const& value,
                                    Comparator cmp) {
...
template <typename ForwardIter, typename Sentinel, typename T, typename Comparator>
constexpr std::ptrdiff_t count_sentinel(ForwardIter start, Sentinel sentinel, T const& value,
                                        Comparator cmp) {
...
template <typename ForwardIter1, typename Sentinel1, typename ForwardIter2, typename Sentinel2,
          typename Comparator>
constexpr bool check_element_counts(ForwardIter1 first_1, const Sentinel1 end_1,
                                    ForwardIter2 first_2, const Sentinel2 end_2, Comparator cmp) {
    auto cursor = first_1;
    while (cursor != end_1) {
...
template <typename ForwardIter1, typename Sentinel1, typename ForwardIter2, typename Sentinel2,
          typename Comparator>
constexpr bool is_permutation(ForwardIter1 first_1, const Sentinel1 end_1, ForwardIter2 first_2,
                              const Sentinel2 end_2, Comparator cmp) {
    // TODO: no optimization for stronger iterators, because we would also have to constrain on
    // sentinel vs not sentinel types
...
class ReporterRegistry {
   ...
   public:
    ...
    std::map<std::string, IReporterFactoryPtr, Detail::CaseInsensitiveLess> const& getFactories()
        const;
...
class RunContext final : public IResultCapture {
   ...
   public:  // IResultCapture
    ...
    void handleMessage(AssertionInfo const& info, ResultWas::OfType resultType,
                       std::string&& message, AssertionReaction& reaction) override;
    ...
    bool sectionStarted(StringRef sectionName, SourceLineInfo const& sectionLineInfo,
   ...
   public:
    class ScopedElement {
       public:
        ...
        ScopedElement& writeText(StringRef text, XmlFormatting fmt = XmlFormatting::Newline |
...
template <typename T>
using is_generic_matcher = std::is_base_of<Catch::Matchers::MatcherGenericBase,
...
template <typename T>
using is_matcher = std::is_base_of<Catch::Matchers::MatcherUntypedBase,
                                   std::remove_cv_t<std::remove_reference_t<T>>>;
...
template <typename T, typename... MatcherTs, std::size_t N, typename Arg, std::size_t Idx,
          std::size_t... Indices>
...
class MatchAllOfGeneric final : public MatcherGenericBase {
   public:
    ...
    friend MatchAllOfGeneric<MatcherTs..., MatchersRHS...> operator&&(
        MatchAllOfGeneric<MatcherTs...>&& lhs, MatchAllOfGeneric<MatchersRHS...>&& rhs) {
...
class MatchAnyOfGeneric final : public MatcherGenericBase {
   public:
    ...
    friend MatchAnyOfGeneric<MatcherTs..., MatchersRHS...> operator||(
        MatchAnyOfGeneric<MatcherTs...>&& lhs, MatchAnyOfGeneric<MatchersRHS...>&& rhs) {
...
template <typename Matcher>
std::enable_if_t<Detail::is_matcher<Matcher>::value, SizeMatchesMatcher<Matcher>> SizeIs(
    Matcher&& m) {
 ...
 */
template <typename T>
std::enable_if_t<!Detail::is_matcher<T>::value, ContainsElementMatcher<T, std::equal_to<>>>
Contains(T&& elem) {
...
template <typename Matcher>
std::enable_if_t<Detail::is_matcher<Matcher>::value, ContainsMatcherMatcher<Matcher>> Contains(
    Matcher&& matcher) {
...
class UnorderedRangeEqualsMatcher final : public MatcherGenericBase {
   ...
   public:
    ...
    constexpr bool match(RangeLike&& rng) const {
        using std::begin;
        using std::end;
        return Catch::Detail::is_permutation(begin(m_desired), end(m_desired), begin(rng), end(rng),
                                             m_predicate);
    }
...
constexpr RangeEqualsMatcher<std::initializer_list<T>, Equality> RangeEquals(
...
template <typename T>
struct has_description<T, void_t<decltype(T::getDescription())>> : std::true_type {};
